...
function renderCity1map(city, country) {
  ...
  document.querySelector('.wiki-city1').setAttribute("href", "https://es.wikipedia.org/wiki/" + city);
}
...
function renderCity2map(city, country) {
  ...
  document.querySelector('.wiki-city2').setAttribute("href", "https://es.wikipedia.org/wiki/" + city);
}
...
// We load the cities within the drop-down buttons (two cities)
...
function loadCities () {
  ...
  for (let i = 0; i < cityLatLong.length; i++) {
    ...
    querySelected = document.querySelector('.city2');
    node = document.createElement('option');
    // console.log (node);
    node.setAttribute("value", cityLatLong[i].cityname);
    // console.log("city: ", cityLatLong[i].cityname)
